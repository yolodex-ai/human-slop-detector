...
console.log(
  `tested against ${humanLabels.length} real-world examples with human-labeled ground truth:\n`
);
console.log(`| metric | gpt-5.1 | human-slop-detector |`);
console.log(`|--------|---------|---------------------|`);
console.log(
  `| precision | ${fmt(gptMetrics.precision)} | ${fmt(libMetrics.precision)} |`
);
console.log(
  `| recall | ${fmt(gptMetrics.recall)} | ${fmt(libMetrics.recall)} |`
);
console.log(`| f1 score | ${fmt(gptMetrics.f1)} | ${fmt(libMetrics.f1)} |`);
console.log(
  `| accuracy | ${fmt(gptMetrics.accuracy)} | ${fmt(libMetrics.accuracy)} |`
);
...
console.log(`\n### detailed breakdown\n`);
console.log(
  `**gpt-4.1:** ${gptMetrics.tp} TP, ${gptMetrics.fp} FP, ${gptMetrics.tn} TN, ${gptMetrics.fn} FN`
);
console.log(
  `**library:** ${libMetrics.tp} TP, ${libMetrics.fp} FP, ${libMetrics.tn} TN, ${libMetrics.fn} FN`
);
